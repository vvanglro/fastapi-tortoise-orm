...
@app03.get('/query')
def page_limit(page: int = 1, limit: Optional[int] = 10):  # 给了默认值就是选填参数，没给默认值就是必填参数
    if limit:
...
def page_limit(param: bool = False):  # bool类型转换： yes on 1 True true会转换成true
...
def query_params_validate(
        value: str = Query(..., min_length=8, max_length=16, regex="^a"),  # regex 匹配以a开头的字符串 且最少长度为8
