...
'''OAuth2 密码模式和 FastAPI的 OAuth2PasswordBearer'''
...
当请求到来的时候，FastAPI会检查请求的Authorization头信息，如果没有找到Authorizaion头信息,或者头信息的内容不是Bearer token，它会返回401状态码(unauthorized)
'''

oauth2_schema = OAuth2PasswordBearer(tokenUrl='/chapter06/token')  # 请求Token的URL地址：http://ip:port/chapter06/token
...
# pwd_context.hash(secret='secret')
