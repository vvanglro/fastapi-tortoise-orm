...
async def create_city(city: schemas.CreateCity):
    ...
    if db_city:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail='城市已存在',
        )
...
async def update_city(city: str, city_data: schemas.CreateCity):
    ...
    if db_city is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail='城市未找到',
        )
    # 更新数据是返回的更新的行数 这里是更加城市名去更新 由于城市名是unique所以只返回1或0
    num = await crud.update_city(city_name=city, city=city_data)
    if not num:
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY, detail='数据更新失败',
        )
